data {
    ...
    int cat_map[n_obs, 2];              // mapping of predictor values to categories

    // data
    int<lower=1, upper=n_cat> y[n_rep];                      // response variables
...
transformed parameters {
    ...
    for (i in 1:n_obs){
      y_pred_cat[cat_map[i, 1], cat_map[i, 2]] = y_pred[i];
    }
}
model {
    // priors:
    ...
    // observations:
    for (i in 1:n_rep) {
      y[i] ~ categorical_logit(y_pred_cat[i]');
    }
}
