data {
    ...
    int<lower=0, upper=1> coef_b[n_coef];
    int g_per_ff[n_coef];               // # groups per fixed effect
    int sig_beta_map[sum(g_per_ff)];
...
parameters {
    real alpha[sum(coef_b)];                         // fixed effects and intercepts
    ...
    real<lower=0> sig_beta[max(sig_beta_map)];      // group standard deviations
